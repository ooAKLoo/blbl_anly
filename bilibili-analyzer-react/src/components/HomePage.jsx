...
function HomePage({ savedUpList = [], upDataMap = {}, onLoadUpData, onViewUpDetail }) {
  ...
  const renderScatterChart = () => {
    ...
    const series = selectedUps.map(up => {
  ...
  }, [selectedUps, selectedTimeRange, customStartDate, customEndDate, upDataMap]);
        ...
        {selectedUps.length > 0 && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-medium text-neutral-700">播放量时间轴对比</h3>
                <div className="flex items-center gap-4">
                  {selectedUps.map(up => (
              ...
              <div ref={scatterChartRef} className="h-[400px]"></div>
              <p className="text-xs text-neutral-400 mt-2">点击圆点可查看视频详情并跳转</p>
            </div>

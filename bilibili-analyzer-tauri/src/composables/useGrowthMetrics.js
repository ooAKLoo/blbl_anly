...
export function useGrowthMetrics(videos) {
  ...
  const sortedVideos = computed(() => {
    ...
    return [...videos.value].sort((a, b) => new Date(a.publish_time) - new Date(b.publish_time));
  });
  ...
  // 场均播放
  const avgPlay = computed(() => {
    if (!videos.value || videos.value.length === 0) return 0;
    return totalPlays.value / videos.value.length;
  });

  // 最高播放视频
  const maxPlayVideo = computed(() => {
    if (sortedVideos.value.length === 0) return null;
    return sortedVideos.value.reduce((max, v) => v.play_count > max.play_count ? v : max, sortedVideos.value[0]);
  });

  // 成长倍数（前10个 vs 后10个视频的均播对比）
  const growthMultiple = computed(() => {
    if (sortedVideos.value.length < 10) return 1;
  ...
  const creatorTags = computed(() => {
    ...
    // 基础数据
    const avg = avgPlay.value;
    const firstPlay = firstVideoPlays.value;
    const maxPlay = maxPlayVideo.value?.play_count || 0;
    ...
    const danmuRate = totalDanmu / (totalPlays.value || 1);
    const commentRate = totalComments / (totalPlays.value || 1);

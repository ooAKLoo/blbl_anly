...
import { ACCOUNT_LEVEL_THRESHOLDS, DURATION_BINS, TREND_THRESHOLDS, ANALYSIS_THRESHOLDS } from '../constants';
...
export function useVideoMetrics(analysisVideos, allVideos = null) {
  ...
  const allVideosAvgPlays = computed(() => {
    if (!allVideos || !allVideos.value || allVideos.value.length === 0) return 0;
    const total = allVideos.value.reduce((sum, v) => sum + v.play_count, 0);
    return Math.round(total / allVideos.value.length);
  });
